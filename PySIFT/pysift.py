...
def computeKeypointsAndDescriptors(image, sigma=1.6, num_intervals=3, assumed_blur=0.5, image_border_width=5):
    ...
    t2 = time.time()
    print("it takes {}s to generate pyramid images".format(t2 - t1))
    keypoints = findScaleSpaceExtrema(gaussian_images, dog_images, num_intervals, sigma, image_border_width,
                                      contrast_threshold=0.001)
    keypoints = removeDuplicateKeypoints(keypoints)
    keypoints = convertKeypointsToInputImageSize(keypoints)
    t3 = time.time()
    print("it takes {}s to generate keypoints".format(t3 - t2))
    ...
    for gaussian_images_in_octave in gaussian_images:
        ...
        for first_image, second_image in zip(gaussian_images_in_octave, gaussian_images_in_octave[1:]):
            dog_images_in_octave.append(subtract(second_image, first_image,
                                                 dtype='float32'))  # ordinary subtraction will not work because the images are unsigned integers
...
def localizeExtremumViaQuadraticFit(i, j, image_index, octave_index, num_intervals, dog_images_in_octave, sigma,
                                    contrast_threshold, image_border_width, eigenvalue_ratio=10000,
...
###############################
# Saving and loading keypoint & descriptor #
###############################
def saveKeypointsToFile(i, kp):
    '''Save the i-th keypoint to file'''
    deserializedKeypoints = []
    filepath = "keypoints" + str(i) + ".txt"
    for point in kp:
...
def saveDescriptorsToFile(i, des):
    '''Save the i-th descriptor to file'''
    filepath = "descriptors" + str(i) + ".txt"
    with open(filepath, 'wb') as fp:
        pickle.dump(des, fp)


def fetchKeypointFromFile(i):
    '''Fetch the i-th keypoint to file'''
    filepath = "keypoints" + str(i) + ".txt"
    keypoint = []
    ...
    for point in deserializedKeypoints:
        temp = KeyPoint(x=point[0][0], y=point[0][1], size=point[1], angle=point[2], response=point[3], octave=point[4],
                        class_id=point[5])
...
def fetchDescriptorFromFile(i):
    '''Fetch the i-th descriptor to file'''
    filepath = "descriptors" + str(i) + ".txt"
    file = open(filepath, 'rb')
...
def templateMatching(img1, img2, kp1, des1, kp2, des2, threshold=0.7):
    '''Matching two images using FLANN'''
