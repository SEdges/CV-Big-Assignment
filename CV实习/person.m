...
clc;
%读取两幅图片
img_1 = imread('VGG_Oxford_graf01.jpg');
img_2 = imread('VGG_Oxford_graf03.jpg');
...
% 将特征点信息写入Excel表格
filename = 'person.sift_features1.xlsx';
writetable(points_info1, filename);

filename = 'person.sift_features2.xlsx';
writetable(points_info2, filename);


%% Question Two：SIFT特征匹配
%匹配两幅图像中的特征点
indexPairs=matchFeatures(features1, features2, 'MaxRatio', 0.3, 'MatchThreshold', 50);
%'MaxRatio=0.3'：当最佳匹配和次优匹配之间的比率小于0.3时，才将匹配视为有效匹配
